...
// Слушает текущее событие 'mousewheel' и принимает объект scroller. Запускается из 'manageEvents' строка 686
baron.toCallbackWheel = function(e, scroller) {
	if(scroller !== undefined) callWheel(e, scroller);
}

// Слушает текущее событие 'onscroll' и принимает объект scroller. Запускается из 'manageEvents' строка 585
baron.toCallbackScroll = function(e, scroller) {
	obj_scroller = scroller;
	if (scroller !== undefined) callScroll(e, scroller);
}

// Слушает событие инициализации объекта baron и принимает объект scroller. Запускается из 'constructor' строка 1201
...
module.exports = function fix(userParams) {
	...
	//console.log('0 MOUSE WHEEL: ', scroller)
        try {
	    ...
	    //console.log('MOUSE WHEEL: ', scroller)
        } catch (ex) {
            //
        }
    }
